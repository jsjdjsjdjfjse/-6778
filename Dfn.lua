    ...
    if (hp / maxHp) * 100 < Settings.healThreshold and hp > 0 then
        ...
        if now - lastHealTime >= Settings.healSpeed then
            ...
            if Settings.useSnakeOil then
                local snakeId = FindItemId("Snake Oil") or FindItemId("蛇油")
                if snakeId then
                    pcall(function()
                        useEvent:FireServer(snakeId)
                        lastHealTime = now
                        used = true
                        print("💚 已喝蛇油，血量: " .. math.floor(hp))
                    end)
                end
            end
            if not used and Settings.useBandage then
                local bandageId = FindItemId("Bandage") or FindItemId("绷带")
                if bandageId then
                    pcall(function()
                        useEvent:FireServer(bandageId)
                        lastHealTime = now
                        used = true
                        print("🩹 已用绷带，血量: " .. math.floor(hp))
                    end)
                end
            end
            if not used and Settings.useSnakeOil and not Settings.useBandage then
                local snakeId = FindItemId("Snake Oil") or FindItemId("蛇油")
                if snakeId then
                    pcall(function()
                        useEvent:FireServer(snakeId)
